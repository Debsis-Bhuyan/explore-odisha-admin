...
import { useFollowers } from "../hooks/follower_hook";
      ...
      <Table highlightOnHover withTableBorder className="flex-1">
        ...
        <Table.Tbody>
          {/* {data?.data?.map(({_id, followerId,createdAt})=(
            <Table.Tr key={_id} >dfdf</Table.Tr>
          ))} */}
          {data?.data?.map(({ _id, followerId, createdAt }) => {
            <Table.Tr
              key={_id}
            ...
            >
              <Table.Td className="flex gap-2 items-center">
                {followerId.image ? (
                  ...
                  <>
                    <p className="w-10 h-10 rounded-full items-center justify-center bg-indigo-700 text-white">
